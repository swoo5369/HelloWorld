...
int main()
{
...

	{
		...
		result = (a > 0 && --b > 0) ? true : false;// 앞의 값이 참이 아닐때 && 뒤의 식을 실행 안함 (축약논리) b = -5
		result = !(a > 0) ? true : false; //! 값을 반대로 바꿈
		cout << "Result = " << boolalpha << result << noboolalpha << " b= " << b << endl;
	}

	{
		int num = 10;
		cout << "2진법 표기 : " << bitset<32>(num) << endl; 32개 자리를 표시
		...
		cX = cX >> 2;//오른쪽으로 두개 민다 맨 앞비트가 복사된다
		cout << "cX= " << bitset<8>(cX) << endl;
		//암호화에 사용
